...
## 2026-05-03 - PWAの`navigateFallback`がFirebase Auth handlerを潰す問題

カスタムドメイン `podcast.tachibanayu24.com` を authDomain にしたら、`signInWithPopup` で「認証で問題が発生しました」 (`auth/internal-error`) → 再試行で「not found」になった。

原因は2段。

1. **OAuth Client 未登録**: `redirect_uri_mismatch`。GCP Console > Credentials の OAuth 2.0 クライアントに JS 生成元と redirect URI を手動追加 (https://podcast.tachibanayu24.com / https://podcast.tachibanayu24.com/__/auth/handler)。Firebase は default authDomain (`*.firebaseapp.com`) しか面倒見ない。

2. **Service Workerが`/__/auth/handler` を index.html で食う**: `vite-plugin-pwa` の `workbox.navigateFallback: "/index.html"` だけ書いてて denylist が無かった。popup で `__/auth/handler` を navigation 取得しに行くと、SW が SPA shell を返してしまい認証フローが死ぬ。`navigateFallbackDenylist: [/^\/__\//, /^\/api\//]` を追加して回避。

PWA + Firebase Auth (custom domain) では denylist 必須。気をつけたい。

## 2026-05-04 - 文字起こしタイムアウトとダウンロード Failed to fetch

### 文字起こしタイムアウト
`httpsCallable` のクライアント側デフォルト timeout は 70 秒。サーバ側 `transcribeEpisode` は `timeoutSeconds: 540` で、実際 2-3 分で完走するエピソードでも 70 秒で `deadline-exceeded` になりユーザには「時間がかかりすぎたため中止しました」と表示されていた。Cloud Functions ログ側は成功してるのに UI 側だけ失敗していた。

`web/src/lib/functions.ts` で `httpsCallable(functions, name, { timeout: ... })` を server 側 timeoutSeconds に合わせて明示。LLM 系は全部やる。

ついでに transcribe.ts で `temperature: 0.1` と `maxOutputTokens: 32768` を明示し、ingest 結果の content-type を `mediaType` に渡すように修正(以前は `audio/mpeg` ハードコード)。

### Failed to fetch でダウンロードできない
`offline.ts > downloadAudio` がポッドキャスト配信元へ直接 `fetch()` していた。多くの配信ホストは CORS 返さないので browser fetch だけ NG。`<audio>` タグは CORS 不要なので playback は問題なく動いていた。

対策として `audioProxy` Cloud Function を新設(`functions/src/audio-proxy.ts`)。Firebase ID トークンで認証 → Firestore から episode を引き → safeFetch で配信元から取得 → streaming pipe。Hosting rewrite `/api/audio` → `audioProxy` を追加。`offline.ts` はこれを叩く形に変えた。

音声プレイヤー側は元 URL のまま(audio タグなので CORS 不要、配信元の bandwidth 食わせるほうが Functions egress より安い)。proxy はあくまでオフライン保存用。
